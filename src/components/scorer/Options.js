    		...
    		<ul className="content-list" onChange={(evt) => props.setNumPlayers(evt)}>
				{
					NUMPLAYERS.map((num) => (
						<li key={num} className="content-item content-item-5">
  						<input type="radio" name="numPlayers" value={`${num}`} id={`opt-p-${num}`} />
  						<label htmlFor={`opt-p-${num}`}>
  							<img src={`/images/options/options-${num}.svg`} alt={`${num}-players`} />
  							<span className="label-text">{`${num}`}<span className="label-checkmark" /></span>
  						</label>
  					</li>
					))
				}
  			...
  			<ul className="content-list" onChange={(evt) => props.setExpansions(evt)}>
  			{
					EXPANSIONS.map((name) => (
						<li  key={name} className="content-item content-item-3">
  						<input type="checkbox" className="btn" name="expansions" value={`${name}`} id={`opt-p-${name}`} />
  						<label htmlFor={`opt-p-${name}`}>
  							<img src={`/images/options/options-${name}.svg`} alt={`Expansion ${name}`} />
  							<span className="label-text">{`${name}`}<span className="label-checkmark" /></span>
  						</label>
  					</li>
					))
				}
